...
All new files you create should have `from iafisher.prelude import *`.
...
- Use `timehelper.now()` from `iafisher.timehelper` to get the current time as an aware datetime for the
  local timezone.
- Use `LOG` from the prelude for logging.
- Use `iafisher.tabular` to print output in a tabular format.
...
`iafisher` is a separate repository whose source code you don't have access to.
...
- For human-facing durations, sizes, and times, use `lib/humanunits` (e.g., `parse_duration`, `parse_bytes`, `parse_time`) and `iafisher.timehelper` (`now()`, `today()`, `TZ_NYC`, `TZ_UTC`) instead of manual parsing/formatting.
